#!/usr/bin/env python3
"""
Batch-embed CTE names from PostgreSQL via OpenRouter (qwen/qwen3-embedding-4b) and store vectors in Qdrant.
...
Usage:
    ...
    python scripts/embed_cte_openrouter.py
    python scripts/embed_cte_openrouter.py --batch-size 500 --embed-batch 64 --collection cte_name_openrouter
    python scripts/embed_cte_openrouter.py --reset  # ignore checkpoint, start from scratch
"""
...
MAX_RETRIES = 5
RETRY_DELAY = 5
CHECKPOINT_FILE = Path(__file__).parent / ".embed_checkpoint_openrouter.json"
...
async def get_embedding_dim(http: httpx.AsyncClient) -> int:
    resp = await http.post(
        ...
        json={"model": EMBEDDING_MODEL, "input": ["test"], "dimensions": EMBEDDING_DIM},
        timeout=30,
    )
...
async def main(batch_size: int, collection: str, embed_batch_size: int, reset: bool):
    if reset:
        clear_checkpoint()
    ...
    try:
        ...
        async with httpx.AsyncClient() as http:
            while True:
                ...
                for i in range(0, len(texts_lower), embed_batch_size):
                    chunk = texts_lower[i : i + embed_batch_size]
                    chunk_ids = ids[i : i + embed_batch_size]
                    try:
                        ...
                        if hasattr(e, "response") and getattr(e.response, "text", None):
                            err_msg = f"{e} | response: {e.response.text[:500]}"
                        logger.error(f"Embedding failed at batch offset {i}: {err_msg}")
...
async def run_with_restart(
    ...
    for attempt in range(1, max_restarts + 1):
        try:
            await main(batch_size, collection, embed_batch_size, reset)
            return
        except Exception as e:
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Embed CTE names → Qdrant (OpenRouter qwen/qwen3-embedding-4b)")
    p.add_argument("--batch-size", type=int, default=32, help="Rows per DB fetch")
    p.add_argument("--embed-batch", type=int, default=EMBED_BATCH_SIZE, help="Texts per OpenRouter embed call (default 64)")
    p.add_argument("--collection", default="cte_name_openrouter", help="Qdrant collection")
    ...
    asyncio.run(run_with_restart(
        args.batch_size, args.collection, args.embed_batch,
        args.reset, args.max_restarts, args.restart_delay,
    ))
